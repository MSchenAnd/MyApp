...
//		m_nItemHeight = (display[1]-CoreUtil.dipToPixel(getContext(),178))/l.length;
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if(isFirst){
			m_nItemHeight = (h-CoreUtils.dipToPixel(getContext(),12))/l.length;
			isFirst = false;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setColor(getResources().getColor(R.color.sidebar));
		paint.setTextSize(CoreUtils.dipToPixel(getContext(),13));
